...
const debug = (elves) => {
  ...
  const level = range(minX, maxX).map((x) => range(minY, maxY).map((y) => elves.map(e => e.toString()).includes([x, y].toString()) ? '#' : '.').join('')).join('\n')
...
const both = pipe(
  split('\n'),
  map(split('')),
  reduce([], (acc, row, x) => reduce(acc, (innerAcc, point, y) => {
  ...
  elves => {
    ...
    const needsToMove = (elfPos) => Object.values(directions).some(dir => occupiedPositions.has(translate(elfPos, dir).toString()))
    const findMoveIntent = (elfPos) => directionOrder.find(dir => adjacentDirections[dir].every(d => !occupiedPositions.has(translate(elfPos, directions[d]).toString())))
    const moveElves = (elvesToMove) => {
      ...
      const fistStageResult = elvesToMove
        ...
        .map(([elfPos, direction]) => [elfPos, translate(elfPos, directions[direction] ?? [0, 0])])
      ...
      return fistStageResult.map(([elfPos, nextPos]) => conflictPositions.has(nextPos.toString()) ? elfPos : nextPos);
    }
