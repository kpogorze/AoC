...
const first = pipe(
  ...
  map((arr, i) => {
    ...
    let operation = (n) => expression[1] === '+' ? getFirst(n) + getSecond(n) : getFirst(n) * getSecond(n)
    let getFirst = n => (isNaN(toInt(expression[0])) ? n : toInt(expression[0]))
    let getSecond = n => (isNaN(toInt(expression[2])) ? n : toInt(expression[2]))
