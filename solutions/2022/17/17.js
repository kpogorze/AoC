...
const checkPoint = (chamber) => ([x,y]) => {
  if(!chamber[x]) chamber[x] = emptyRow();
  return chamber[x][y];
}

const fillPoint = (chamber, char = '#') => ([x,y]) => {
...
const debug = (chamber, figure = []) => {
  ...
  console.log(temp.reverse().map(l => (l ?? emptyRow()).join('')).join('\n'))
}

const newFloorPoints = (chamber) => {
  const searchDirection = [[0,1], [0,-1], [-1,0]]
  const touchedRocks = [];
  ...
  while(toCheck.length) {
    ...
    if (nextMoves.map(checkPoint(chamber)).some(point => point === '-')) return;
...
const first = pipe(
  ...
  jets => {
    ...
    for(let i=0;i<2022;i++) {
      ...
      let figure = figures[i%figures.length].map(delta => translate(delta, spawnPoint));
      ...
      while(true) {
        ...
        if(newValues.every(point => point === '.')) {
          ...
          localMaxHeight = Math.max(localMaxHeight, Math.max(...figure.map(([x,y]) => x)))
...
const toStringState = (chamber) => {
  return chamber.filter(row => row.some(val => val === '#')).map(row => row.join('')).join('')
}
...
pipe(
  ...
  jets => {
    ...
    for(let i=0;true;i++) {
      ...
      const newLocalMax = Math.max(localMaxHeight, Math.max(...figure.map(([x,y]) => x)))
      ...
      if(newFloor) {
        ...
        if(chamberStates.has(chamberStateKey)) {
          ...
          if((i%figures.length === figureNo) && (jetOrder%jets.length) === jetNo ) {
            console.log('Cycle detected', i, i-chamberStates.get(chamberStateKey)[0], chamberStates.get(chamberStateKey), [i, i%figures.length, jetOrder%jets.length])
            return [chamberStates.get(chamberStateKey)[0], i-chamberStates.get(chamberStateKey)[0], elevationGains];
          }
        }

        chamberStates.set(chamberStateKey, [i, i%figures.length, jetOrder%jets.length])
      }
  ...
  ([firstCycleIndex, cycleLength, elevationGains]) => {
    ...
    const cycleGain = sum(elevationGains.slice(firstCycleIndex, firstCycleIndex+cycleLength));
    const iterations = 1000000000000 - firstCycleIndex;

    return initGain + Math.floor(iterations/cycleLength) * cycleGain + sum(elevationGains.slice(firstCycleIndex, firstCycleIndex + iterations%cycleLength))
  },
);
