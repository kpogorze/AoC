import { reduce, translate, range, lcm, pipe, split, map, log } from "../../../utils.js"
...
const extractBlizzards = reduce([], (acc, row, x) => reduce(acc, (innerAcc, point, y) => {
  if (point === '^') innerAcc.push([[x, y], [-1, 0]])
  if (point === '>') innerAcc.push([[x, y], [0, 1]])
  if (point === '<') innerAcc.push([[x, y], [0, -1]])
  if (point === 'v') innerAcc.push([[x, y], [1, 0]])

  return innerAcc
})(row))

const moveBlizzard = ([rows, cols, blizzards]) => blizzards.map(([spot, dir]) => {
...
const findFreePostitions = ([rows, cols, blizzards]) => {
  const allSpots = range(0, rows - 1).flatMap(x => range(0, cols - 1).map(y => [x, y]));
  const takenSpots = new Set(blizzards.map(([spot, directions]) => spot.toString()))
  return new Set([[-1, 0], [rows, cols - 1], ...allSpots.filter(spot => !takenSpots.has(spot.toString()))].map(i => i.toString()))
}
...
const extractFreePositions = ([rows, cols, blizzards]) => {
  ...
  return [rows, cols, blizzardPostitions.map(blizzardPos => findFreePostitions([rows, cols, blizzardPos]))]
}

const findLowestTravelTime = (startPos, destinationPos, startMinutes, freePositions) => {
  ...
  while (toCheck.length) {
    ...
    directions
      ...
      .filter(pos => freePositions[(minute + 1) % cycleLength].has(pos.toString()))
...
const second = pipe(
  ...
  ([rows, cols, freePositions]) => {
    ...
    const secondTravelTime = findLowestTravelTime(finish, start, firstTravelTime, freePositions)
    const thirdTravelTime = findLowestTravelTime(start, finish, secondTravelTime, freePositions)
