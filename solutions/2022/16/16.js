...
const toNonZeroValvesGraph = valves => {
  const actualValves = Object.entries(valves).filter(([name, [pressure]]) => pressure).map(([name]) => name)
  ...
  return Object.fromEntries(
    ...
    ...actualValves.map((valveName) => [valveName, [valves[valveName][0], timeToOtherValves(valveName)]])]
  )
}
...
const second = pipe(
  ...
  graph => {
    ...
    const calculateScore = memoize((path, timeLeft = time + 1) => {
      ...
      return pressure * (timeLeft - 1) + (rest.length ? calculateScore(rest, timeLeft - 1 - distances[rest[0]]) : 0);
    })

    const calculateCombinedScore = ([first, second]) => calculateScore(first) + calculateScore(second)

    const timeIncrement = (path, to) => graph[path[path.length - 1]][1][to] + 1;

    const checkBestPath = ([firstPath, secondPath, firstTimePassed = 0, secondTimePassed = 0]) => {
      const remainingValves = valvesToCheck.filter(valve => !(firstPath.includes(valve) || secondPath.includes(valve)))
      const valvesAvailableToFirst = remainingValves
        .map(valve => [valve, firstTimePassed + timeIncrement(firstPath, valve)])
