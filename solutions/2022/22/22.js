import { filter, log, map, pick, pipe, range, reduce, split, toInts, translate, zip } from "../../../utils.js";
...
const getEdgePoints = (startingPoint, dir) => edgePoints.map(i => directions[dir].map(dim => dim * i)).map(delta => translate(startingPoint, delta));

const getPairwiseMapping = (firstEdge, firstDir, secondEdge, secondDir) => [
  ...zip(firstEdge.map(point => [point, firstDir]), secondEdge.map(point => [point, secondDir])),
  ...zip(secondEdge.map(point => [point, (secondDir + 2) % directions.length]), firstEdge.map(point => [point, (firstDir + 2) % directions.length])),
];

[
  ...getPairwiseMapping(getEdgePoints([50, 50], DOWN), LEFT, getEdgePoints([100, 0], RIGHT), DOWN),
  ...getPairwiseMapping(getEdgePoints([0, 50], DOWN), LEFT, getEdgePoints([149, 0], UP), RIGHT),
  ...getPairwiseMapping(getEdgePoints([0, 50], RIGHT), UP, getEdgePoints([150, 0], DOWN), RIGHT),
  ...getPairwiseMapping(getEdgePoints([0, 100], RIGHT), UP, getEdgePoints([199, 0], RIGHT), UP),
  ...getPairwiseMapping(getEdgePoints([0, 149], DOWN), RIGHT, getEdgePoints([149, 99], UP), LEFT),
  ...getPairwiseMapping(getEdgePoints([49, 100], RIGHT), DOWN, getEdgePoints([50, 99], DOWN), LEFT),
  ...getPairwiseMapping(getEdgePoints([149, 50], RIGHT), DOWN, getEdgePoints([150, 49], DOWN), LEFT),
...
const second = pipe(
  ...
  ([[level, [rowCount, columnCount]], instructions]) => {
    ...
    instructions.forEach(([ahead, turn]) => {
      ...
      do {
        let nextPosition = translate(currentPosition, directions[directionIndex]);

        if (!level[nextPosition.toString()]) {
          const [otherEdgePosition, otherDirection] = edgeMapping.get([currentPosition, directionIndex].toString())
